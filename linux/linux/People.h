...
class Player : public interObj
{
	...
	int dir = 1;
	Sound knife, getGun;
	SoundBuffer knifeBuffer, getGunBuffer;
public:
	Player(String P, String Name, Level &lev, int W, int H, float X, float Y, int Health, int Sk) : interObj(P, Name, W, H, X, Y)
	{
		...
		l = lev;
		knifeBuffer.loadFromFile("sounds/knife.wav");
		getGunBuffer.loadFromFile("sounds/ak47_draw.wav");
		knife = Sound(knifeBuffer);
		getGun = Sound(getGunBuffer);
	}
...
void Player::Update(float time)
{
	...
	case up: break;
	case change:
		if (chtimer > 1500 && uG.size()>0 ){
			gun_n = (gun_n + 1) % uG.size();
			chtimer = 0;
		} break;
	case stay: break;
	case shoot: if (uG.size() != 0) { 
		if (shtimer > uG[gun_n]->time) { 
			uG[gun_n]->Shoot(l, dir, x, x + w); shtimer = 0; 
		}; break; 
	}
	case shootR:if (uG.size() != 0) { 

		if (shtimer > uG[gun_n]->time) {
			uG[gun_n]->Shoot(l, dir, x - 1, x + w + 1); shtimer = 0;
		}; 
		dx = speed; dir = 1; break; 
	}
